...
module('Integration | Component | star-rating', function (hooks) {
  ...
  test('Renders the full and empty stars correctly', async function (assert) {
      ...
      assert.dom('.fa-star').exists({ count: 4 },
        'The right amount of full stars is rendered');

      assert.dom('.fa-star-o').exists({ count: 1 },
        'The right amount of empty stars is rendered');
  ...
  test('The setRating action', async function(assert) {
    ...
    await render(hbs`{{star-rating rating=song.rating
      onClick=(action 'updateRating' song)}}`);
      await click('[data-test-rr=star-rating-5]');
      assert.equal(this.get('song.rating'), 5,
